...
public class SeriesPanel extends JPanel {
	...
	public SeriesPanel() {
		...
		lblFavoriteTitle = new JLabel("Descubre series");
		...
		collectionInfoBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Sesion.getInstance().setCollectionID(seriesID);
		...
		JLabel lblSelectDisc = new JLabel("Selecciona la serie:");
		lblSelectDisc.setHorizontalAlignment(SwingConstants.CENTER);
		lblSelectDisc.setForeground(new Color(200, 40, 255));
		lblSelectDisc.setFont(new Font("Segoe UI Semibold", Font.PLAIN, 20));
		lblSelectDisc.setBounds(362, 111, 219, 25);
		add(lblSelectDisc);
		...
		addComponentListener(new ComponentAdapter() {
			public void componentShown(ComponentEvent c) {
				try {
					getSeries(Sesion.getInstance().getArtistID());
		...
		comboBox.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					seriesID = listUniqueID[comboBox.getSelectedIndex()];
					getSerieInfo(seriesID);
	...
	private void getSeries(int artistID) throws ClassNotFoundException, SQLException {
		List<Serie> discs = new CollectionController().getSerieByArtist(artistID);
	...
	private void getSerieInfo(int discsID) throws Exception {
		Serie serie = new CollectionController().getSerieInfo(discsID);
		lbldiscsRegDateText.setText(Converter.convertToSimpleDate(serie.getCollectionDate()));
		lbldiscsDescText.setText(serie.getCollectionDesc());
		lbldiscsName.setText(serie.getCollectionName());
		artistImageIcon = serie.getCollectionImage();
